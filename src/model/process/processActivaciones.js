    ...
    llenarDatosRecarga(nip_street, numero, monto, driver) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                let dataLog = { numero, monto, captcha: folio, result: dataRecarga, saldoActual: saldoDisponible, tipo:'recarga' }
                ...
                //logs
                if (error.response) {
                    let dataLog = { numero, monto, captcha: folio, result: error.response, saldoActual: elementos.saldoTotalRecargas, tipo:'recarga' }
                    log.logGenerate(dataLog).then((result) => { }).catch((error) => { })
                    return reject(error)
                }

                let dataLog = { numero, monto, captcha: folio, result: { message: "Error en el proceso en la funcion llenarDatosRecarga" }, saldoActual: elementos.saldoTotalRecargas, tipo:'recarga' }
                log.logGenerate(dataLog).then((result) => { }).catch((error) => { })
                ...
                if (elementos.contadorRecargas <= 12) {
    ...
    confirmarDatosRecarga(driver, validacionMSG) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                let ventanaModal = await driver.wait(until.elementLocated(By.css(selector)), 10000);
                await driver.wait(until.elementIsVisible(ventanaModal), 10000);
...
module.exports = ProcessActivaciones